...
<main class="grid grid-row gap-y-8 items-center justify-items-center pb-8" >
	...
	<article class="grid grid-row gap-y-16 mx-4 items-center justify-items-center">
		<!-- <section class="grid md:grid-cols-2 gap-y-2 justify-items-center">
			<figure>
				<img src={urlFor(oSensei[0].image.asset).width(800).height(1200)} alt="O'Sensei in ceremonial garb" title="{oSensei[0].name}" width="400" height="600"
				class="w-full max-h-screen"/>
				
			</figure>
			<div class="p-12 flex flex-col justify-center">
				<h2 class="font-Roboto text-gray-900 text-4xl  mb-4">{oSensei[0].name}</h2>
				<p class="mt-4 text-gray-900 text-lg leading-relaxed">{oSensei[0].bio}</p>
